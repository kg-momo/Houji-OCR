...
モデルとデータセットのディレクトリを受け取り推論を行うプログラム
...
class Inferencer:
    ...
    def gen(self, dataset, keep_remain=False, with_tqdm=False):
        ...
        # モデルの可視化
        print("\nInput Size ===========> ",
              [(1, opt.input_channel, opt.imgH, opt.imgW),
               (1, opt.batch_max_length+1)])
        #summary(self.model, [(1, opt.input_channel,
        #                      opt.imgH, opt.imgW),
        #                     (1, opt.batch_max_length+1)], depth=10)
        ...
        with torch.no_grad():
            for image_tensors, labels, data in demo_loader:
                ...
                #print(length_for_pred.size(), text_for_pred.size())
                ...
                if 'CTC' in self.opt.Prediction:
                    ...
                    # 推論結果を行ごとに表示
                    for string in preds_str:
                        #print(string)
                        # "〓"が含まれるとき，その位置を表示
                        x_index=[i for i, x in enumerate(string) if x == "〓"]
                        #if x_index:
                        #    print(x_index)
...
if __name__ == '__main__':       
    ...
    g.add_argument('--db_path', required=True, nargs='+',
                   help='データベースへのパス(複数指定可). synthの場合はfont pathを指定する')
    g.add_argument('--db_type', choices=['xmlraw', 'xmllmdb', 'synth'],
                   help='データベースの種類', default='xmlraw')
    g.add_argument('--line_index', type=int, nargs='+',
                   default=None,
                   help='指定の行のみに対して推論. xmllmdb使用時のみ有効')
    action = parser.add_argument_group()
    action.add_argument('--diff', nargs='?', default='none',
                        const='wrong', choices=['none', 'wrong', 'all'],
                        help='差分表示. 画像出力したい場合にはoutimage_dirとfont_pathを指定する')
    action.add_argument('--render', action='store_true',
                        help='diffのgt(正解ラベル)なし版. outimage_dirとfont_pathが必要')
    action.add_argument('--leven', action='store_true',
                        help='normalized edit distance')
    action.add_argument('--acc', action='store_true',
                        help='accuracy')
    action.add_argument('--xml', default=None,
                        help='xml出力を行う先を指定する')
    parser.add_argument('--stat', action='store_true',
                        help='diff指定時の出力を詳細にする')
    parser.add_argument('--outimage_dir', default=None,
                        help='diff指定時の画像出力先')
    parser.add_argument('--font_path', default=None,
                        help='diff指定時画像出力する際に使用するttf font')
    parser.add_argument('--skip_empty', dest='accept_empty',
                        action='store_false', help='GTが空行の推論を行わない')
    ...
    dataset = gen_dataset(opt.db_type,
                          opt.db_path,
                          opt,
                          line_index=opt.line_index,
                          accept_empty=opt.accept_empty,
                          keep_remain=opt.xml)

    # 推論を行う
    generator = Inferencer(opt).gen(dataset,
                                    keep_remain=opt.xml,
                                    with_tqdm=True)

    # ここでTR_WORKERクラスを使って正誤率とか出してる
    char_diff = {
